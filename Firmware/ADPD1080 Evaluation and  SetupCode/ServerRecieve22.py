...
with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as s:
    ...
    with open(csv_filename, "w", newline="") as csvfile:
        ...
        if not file_exists:
            csv_writer.writerow(["Timestamp", "Combined Value1", "Combined Value2"])
                ...
                if data:
                    ...
                    if len(values) == 112:
                        try:
                            ...
                            # Write alternating values in two columns
                            for i in range(0, len(combined_values), 2):
                                timestamp = receive_time_us - ((len(combined_values) - 1 - i) * timestamp_increment)
                                value1 = combined_values[i]
                                value2 = combined_values[i+1] if i+1 < len(combined_values) else ""
